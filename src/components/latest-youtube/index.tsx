    ...
    const fetchVideos = async () => {
      try {
        ...
        if (response.data.status === 'ok') {
          const fetchedVideos = response.data.items.slice(0, 6).map((item: any) => {
             // Extract video ID from link or guid if possible, simple approach here
             const videoId = item.guid.split(':')[2]; 
             return {
                id: videoId,
                title: item.title,
                thumbnail: `https://i.ytimg.com/vi/${videoId}/hqdefault.jpg`, // High quality thumbnail
                link: item.link,
      ...
      <div className="card-body">
        <div className="mx-3 mb-2 flex items-center justify-between">
            <div className="flex items-center">
                <svg className="inline-block w-8 h-8 text-primary mr-4" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
                    <path fillRule="evenodd" d="M21.7 8.037a4.26 4.26 0 0 0-3.019-3.02C16.014 4.3 9.774 4.3 9.774 4.3s-6.24 0-8.907.717A4.26 4.26 0 0 0 .847 8.037a40.56 40.56 0 0 0-.847 8.927 40.56 40.56 0 0 0 .847 8.927 4.26 4.26 0 0 0 3.019 3.02c2.667.683 8.907.683 8.907.683s6.24 0 8.907-.717a4.26 4.26 0 0 0 3.019-3.02 40.56 40.56 0 0 0 .847-8.927 40.56 40.56 0 0 0-.847-8.927ZM10 16.964V10L16 13.5l-6 3.464Z" clipRule="evenodd"/>
                </svg>
                <h5 className="card-title text-lg">
                    Latest Videos
                </h5>
            </div>
             <a href={`https://www.youtube.com/channel/${channelId}`} target="_blank" rel="noreferrer" className="text-opacity-60 hover:text-opacity-100 transition-opacity opacity-50 text-base-content text-sm">
                See All
            </a>
        </div>
