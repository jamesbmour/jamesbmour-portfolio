            ...
            if educations_start != -1:
                ...
                for line in educations_section.split('\n'):
                    if 'institution:' in line and current_edu.get('degree'):
                        ...
                        config_docs.append(Document(
                            ...
                            metadata={
                                "source": "portfolio_config",
                                "type": "education",
                                "institution": current_edu.get('institution', 'Unknown'),
                                "last_updated": datetime.now().isoformat()
                            }
                        ))
                        current_edu = {}
                    ...
                    if 'institution:' in line:
                        ...
                        if start_quote != -1:
                            end_quote = line.rfind("'") if "'" in line else line.rfind('"')
                            current_edu['institution'] = line[start_quote+1:end_quote]
                    elif 'degree:' in line:
                        start_quote = line.find("'") if "'" in line else line.find('"')
                        if start_quote != -1:
                            end_quote = line.rfind("'") if "'" in line else line.rfind('"')
                ...
                if projects_start != -1:
                    ...
                    for line in projects_section.split('\n'):
                        ...
                        if 'title:' in line:
                            ...
                            if start_quote != -1:
                                end_quote = line.rfind("'") if "'" in line else line.rfind('"')
                                current_proj['title'] = line[start_quote+1:end_quote]
                        elif 'description:' in line:
                            start_quote = line.find("'") if "'" in line else line.find('"')
                            if start_quote != -1:
                                end_quote = line.rfind("'") if "'" in line else line.rfind('"')
                                current_proj['description'] = line[start_quote+1:end_quote]
                        elif 'link:' in line:
                            start_quote = line.find("'") if "'" in line else line.find('"')
                            if start_quote != -1:
                                end_quote = line.rfind("'") if "'" in line else line.rfind('"')
